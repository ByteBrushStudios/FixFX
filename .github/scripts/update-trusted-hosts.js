...
async function fetchFiveMListing() {
  return new Promise((resolve, reject) => {
    https
      .get(
        FIVEM_HOSTING_URL,
        { headers: { "User-Agent": "FixFX-TrustedHostsScraper/1.0" } },
        (res) => {
          let data = "";
...
function parseHostingProviders(html) {
  ...
  const linkRegex =
    /<a[^>]*href=["']([^"']*(?:zap-hosting|gtxgaming|nitrado|g-portal|firestorm|nitrado|gameservers|lgsm)[^"']*)["'][^>]*>([^<]+)<\/a>/gi;
  let match;
  ...
  const knownProviders = [
    ...
    {
      id: "firestorm-servers",
      name: "Firestorm Servers",
      url: "https://firestormservers.com",
    },
    {
      id: "gameservers",
      name: "GameServers",
      url: "https://www.gameservers.com",
    },
  ];
