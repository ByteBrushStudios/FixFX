...
<CheckList
  items={[
    "Regular profiling helps identify issues before affecting players",
    "Use appropriate frame counts (500-1000 frames for initial analysis)",
    "Enable profiling only when needed to minimize performance impact",
    "Compare profiles over time to track performance trends",
    "Focus on resources with highest execution times",
    "Make one change at a time between profiles to isolate improvements",
  ]}
  variant="check"
/>

<Callout type="info">
  Regular profiling helps identify performance issues before they affect your
  players. Make it part of your development workflow.
</Callout>
...
<Callout type="warn">
  Profiling can impact server performance. Use appropriate frame counts and
...
<Callout type="info">
  Save profiles before and after optimizations to measure improvement and track
  changes over time.
</Callout>
