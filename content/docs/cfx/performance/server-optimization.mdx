...
<PropertyCard
  title="Recommended Hardware Specs"
  content="Choose CPU with high single-thread performance over core count. Minimum 8GB RAM (16GB+ recommended). SSD storage for better I/O performance. Stable 1 Gbps connection recommended for production servers."
/>
...
<Callout type="info">
  CPU single-thread performance is more important than core count for FiveM
  servers. Choose a CPU with high single-thread performance for the best
  results.
</Callout>
...
4. **Framework resources** (esx\__, qb-_)
5. **Custom resources**
...
<Callout type="warning">
  Always test optimizations in a development environment before applying them to
  your production server. Performance tuning is an iterative process that
