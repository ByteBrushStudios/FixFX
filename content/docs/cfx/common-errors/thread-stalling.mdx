...
<CheckList
  items={[
    "Always include Citizen.Wait() in loops to yield control",
    "Use async database operations instead of synchronous ones",
    "Break heavy operations into smaller chunks across frames",
    "Monitor resource tick times with resmon command",
    "Avoid nested loops without intermediate yields",
    "Use callbacks and promises for long-running tasks",
  ]}
  variant="check"
/>
...
<Callout type="warning">
  Never release resources with known thread stalling issues. Test thoroughly
  under load before deploying to a production server.
</Callout>
