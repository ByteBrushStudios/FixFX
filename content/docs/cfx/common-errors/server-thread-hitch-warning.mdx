...
<TroubleshootingCard
  title="Server Thread Hitch Detection"
  symptoms={[
    "WARNING: Server thread hitch detected",
    "Server becomes unresponsive temporarily",
    "Players experience lag spikes",
    "Resource tick times spike suddenly",
  ]}
  causes={[
    "Heavy synchronous operations blocking the main thread",
    "Long-running loops without proper yielding (Wait)",
    "Excessive database queries without pagination",
    "Complex calculations on the main thread",
    "Too many resources with heavy load",
  ]}
  solutions={[
    "Add Citizen.Wait() or proper yields in loops",
    "Move heavy operations to async callbacks",
    "Optimize database queries and add pagination",
    "Distribute calculations across multiple frames",
    "Profile resources with resmon to identify bottlenecks",
  ]}
/>
...
Optimize your server configuration. Check the [official FiveM documentation](https://docs.fivem.net/docs/server-manual/server-commands/) for current server convars.
...
<Callout type="warning">
  Always monitor your server's performance and address thread hitches promptly
  to prevent server instability. Regular monitoring and maintenance are
  essential for optimal server performance.
</Callout>

<Callout type="info">
  For more information about thread management and performance optimization,
  refer to the CitizenFX documentation and server commands reference.
</Callout>
