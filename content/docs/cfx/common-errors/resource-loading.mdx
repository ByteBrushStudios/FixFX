...
<CheckList
  items={[
    "Ensure fxmanifest.lua exists in resource root directory",
    "Verify all script paths are correct and relative to resource folder",
    "Declare all dependencies in manifest file",
    "Use current manifest format (fx_version 'cerulean')",
    "Check for typos in resource names and file paths",
    "Validate Lua syntax with a linter before deploying",
  ]}
  variant="check"
/>
...
# Check docs.fivem.net for current server convar options
...
<Callout type="warning">
  Always test resource changes in a development environment before deploying to
  production. Regular monitoring and maintenance are essential for optimal
  server performance.
</Callout>

<Callout type="info">
  For more information about resource development, refer to the relevant
  sections in the documentation and server commands reference.
</Callout>
