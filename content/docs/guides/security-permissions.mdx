...
<PropertyCard type="error" name="Critical Security" description="Never share your server license key, database passwords, or administrator credentials. Always use strong, unique passwords and enable two-factor authentication where available." />
...
<StepList
  steps={[
    { title: "Set Default Firewall Policies", description: "Deny all incoming traffic by default, whitelist only necessary ports" },
    { title: "Open FiveM Port", description: "Allow port 30120 (TCP and UDP) for FiveM server connections" },
    { title: "Secure SSH Access", description: "Change SSH from default port 22 and disable root login" },
    { title: "Restrict Database Access", description: "Allow MySQL/MariaDB only from 127.0.0.1 (local connections)" },
    { title: "Enable Firewall", description: "Activate firewall rules with: sudo ufw enable" }
  ]}
...
<PropertyCard type="info" name="ACE Basics" description="ACE (Access Control Entry) is FiveM's permission system that controls who can execute what commands and access what resources. It's configured in server.cfg using a principal-based hierarchy." />
...
<PermissionTable
  permissions={[
    { principal: "identifier.steam:110000112345678", type: "Player", description: "Steam ID based player identifier (most specific)" },
    { principal: "identifier.license:abc123def456", type: "Player", description: "Rockstar license based identifier" },
    { principal: "identifier.discord:123456789012", type: "Player", description: "Discord ID based identifier" },
    { principal: "group.admin", type: "Group", description: "Admin group - highest privilege level" },
    { principal: "group.moderator", type: "Group", description: "Moderator group - moderation permissions" },
    { principal: "builtin.everyone", type: "Built-in", description: "All connected players" },
    { principal: "builtin.console", type: "Built-in", description: "Server console/RCON access" }
  ]}
...
<PropertyCard type="warn" name="Permission Hierarchy" description="Always define deny rules after allow rules. Place more specific permissions before wildcards. Test permission changes in a development environment first." />
