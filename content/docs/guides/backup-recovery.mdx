...
<StepList
  steps={[
    "Identify all critical data categories (server files, database, runtime data)",
    "Establish backup frequency based on data importance",
    "Choose appropriate backup storage location",
    "Test restore procedures before relying on backups",
    "Document backup procedures for team awareness",
...
<CheckList
  items={[
    "Server files: Artifacts, server.cfg, and resource configurations",
    "Database: Player data, vehicle ownership, economy information",
    "Runtime data: Current server state, active sessions",
    "Configuration files: Custom scripts and modifications",
  ]}
/>
