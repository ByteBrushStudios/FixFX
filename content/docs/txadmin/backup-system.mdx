...
<PropertyCard title="Full Backups" content="Complete server backup including all data. Frequency: Weekly or monthly. Enables complete server restoration." />

<PropertyCard title="Incremental Backups" content="Only changed files since last backup. Frequency: Daily or hourly. Requires full backup + incrementals for restore. Minimal storage usage." />

<PropertyCard title="Resource-Only Backups" content="Backup only resources folder. Medium size backups. Best for development servers and pre-update snapshots." />
...
<StepList steps={[
  "Navigate to Settings → Backup in txAdmin panel",
  "Enable Automatic Backups toggle",
  "Set backup frequency (daily, weekly, or custom cron)",
  "Choose backup time during low-activity period",
  "Configure retention policies to manage storage",
  "Select backup storage location (local or cloud)"
]} />
