...
<Callout type="info">
  The Contributors API provides access to GitHub contributor information for the
  CodeMeAPixel organization. Built with Go and Fiber, it fetches real-time data
  from GitHub and caches it for optimal performance.
</Callout>

## Overview

The Contributors API provides access to GitHub contributor information and repository statistics. It helps:

- Display contributor acknowledgments on documentation
- Showcase community involvement in projects
- Track project activity and contributions
- Generate contributor reports
- Build community dashboards

This API fetches data directly from GitHub's API and caches it for performance.

## Base URL

**Production:** `https://core.fixfx.wiki/api/contributors`

**Development:** `http://localhost:3001/api/contributors`

## Authentication

The Contributors API is public and doesn't require authentication. However, it respects GitHub's rate limiting. For higher rate limits, provide a GitHub token via the `GITHUB_TOKEN` environment variable when self-hosting.

## Endpoints

### GET /api/contributors

Retrieves contributor information for the CodeMeAPixel organization repositories.
...
| `limit`   | number | Maximum number of contributors              | `50`             |
...
```json
{
  ...
  "metadata": {
    "total": 15,
    "organization": "CodeMeAPixel",
    "lastUpdated": "2026-01-25T09:00:00Z",
    "cacheExpiry": "2026-01-25T13:00:00Z"
  }
}
```
...
data.data.forEach((contributor, index) => {
  ...
  if (contributor.contribution_types) {
    ...
    console.log(
      `   Pull Requests: ${contributor.contribution_types.pull_requests}`,
    );
  }
});
...
data.metadata.stats.top_repositories.forEach((repo) => {
  console.log(
    `- ${repo.name}: ${repo.contributors} contributors, ${repo.contributions} contributions`,
  );
});
```
...
async function analyzeRepositoryActivity() {
  ...
  Object.entries(results).forEach(([repo, stats]) => {
    ...
    console.log(
      `  Top Contributor: ${stats.topContributor.login} (${stats.topContributor.contributions} contributions)`,
    );
  ...
  const mostActive = Object.entries(results).reduce((a, b) =>
    results[a[0]].contributions > results[b[0]].contributions ? a : b,
  );
...
async function generateContributorBadges() {
  ...
  badges.mostActive = data.data
    ...
    .sort(
      (a, b) => new Date(b.last_contribution) - new Date(a.last_contribution),
    )[0];
  ...
  badges.newcomer = data.data
    ...
    .sort(
      (a, b) => new Date(b.first_contribution) - new Date(a.first_contribution),
    )[0];
  ...
  badges.allRounder = data.data
    ...
    .map((c) => ({
      ...c,
      diversity: Object.keys(c.contribution_types).filter(
        (key) => c.contribution_types[key] > 0,
      ).length,
    }))
...
<Callout type="info">
  The Contributors API helps build community engagement by recognizing valuable
  contributions across our projects.
</Callout>

<Callout type="note">
  Contribution data is sourced from public GitHub activity and may not reflect
  all forms of contribution to our projects.
</Callout>
